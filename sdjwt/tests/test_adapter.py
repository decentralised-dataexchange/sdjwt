...
class TestDataAttributesAdapter(TestCase):
    def test_to_json_schema(self):
        ...
        expect = {
            ...
            "properties": {
                ...
                "vaccine": {
                    "type": "object",
                    "properties": {"medicinalProductName": {"type": "string"}},
                },
            },
        }
    ...
    def test_from_json_schema(self):
        out = DataAttributesAdapter.from_json_schema(
            schema={
                "age": {"type": "number"},
                "vaccine": {
                    "type": "object",
    ...
    def test_from_credential(self):
        out = DataAttributesAdapter.from_credential(
            credential={"age": 25, "vaccine": {"medicinalProductName": "Moderna"}}
        ).data_attributes

        expect = [
            DataAttribute(name="age", data_type="number", value=25),
            DataAttribute(
                name="vaccine.medicinalProductName", data_type="string", value="Moderna"
            ),
        ]
        self.assertEqual(out, expect)
